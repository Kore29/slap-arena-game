...
- [ ] 1.1 Create `GameModeData` ScriptableObject to define mode parameters.
- [ ] 1.2 Implement `LobbySlot` struct and `LobbyManager` logic on top of `SessionManager`.
- [ ] 1.3 Refactor `GameManager` to handle networked match initialization based on mode.

## 2. Dynamic Lobby UI
- [ ] 2.1 Update `MainMenu.uxml` with the 3 mode selection cards.
- [ ] 2.2 Create `Lobby.uxml` with slot list, team buttons, and bot toggle.
- [ ] 2.3 Implement `LobbyController` to handle UI-Network synchronization.

## 3. Bot Management & AI
- [ ] 3.1 Create `BotNameUtility` with a JSON list of 50+ Spanish names.
- [ ] 3.2 Implement Host-side logic for bot filling and player replacement.
